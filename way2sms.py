...
class sms:
	...
	def msgSentToday(self):
		...
		for self.i in self.t:
			...
			if self.i.decode("utf-8").isdecimal():
	...
	def send(self,mobile_no,msg):
		...
		if len(msg)>139 or len(mobile_no)!=10 or not mobile_no.decode("utf-8").isdecimal():	#checks whether the given message is of length more than 139
	...
	def sendLater(self, mobile_no, msg, date, time):				#Function for future SMS feature.
		...
		if len(msg)>139 or len(mobile_no)!=10 or not mobile_no.decode("utf-8").isdecimal():
			return False
